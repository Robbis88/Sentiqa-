...
// LØNNSROMMET
//
// Formen er `Lonnsrom` i lib/lonnskost/rom.ts. Tallene er oppdiktet,
// men regnestykket er det ekte:
//
//     rom = lønnsandel × brutto
//
// der andelen kommer fra BP for nøyaktig den måneden, og bruttoen enten
// er regnskapets (avlagt måned) eller et anslag skalert med hvor mye av
// salget som har kommet inn, kalibrert mot hvor mye av planlagt margin
// stasjonen faktisk treffer.
//
// PROSENTEN FØRST, KRONENE UNDER — samme rekkefølge som produktet.
// Kroner øverst ble lest som «du har så mye igjen å bruke», og det er
// stikk i strid med hvordan tallet virker: at brutto ble høyere enn
// planlagt gir ikke mer lønn å bruke.
// =====================================================================

export type Romlinje = {
  maaned: string
  /** Faktisk lønn som andel av brutto, i prosent. */
  andel: number
  /** Hva BP legger opp til, i prosent. */
  krav: number
  bruttoKr: number
  anslaatt: boolean
}

export const ROM: Romlinje[] = [
  { maaned: 'September', andel: 47.9, krav: 49.2, bruttoKr: 1284000, anslaatt: true },
  { maaned: 'August', andel: 52.4, krav: 51.0, bruttoKr: 1412300, anslaatt: false },
  { maaned: 'Juli', andel: 55.1, krav: 50.4, bruttoKr: 1338900, anslaatt: false },
  { maaned: 'Juni', andel: 49.8, krav: 50.1, bruttoKr: 1361400, anslaatt: false },
]

/** Leddene i anslaget, i den rekkefølgen de regnes. */
export const ROM_LEDD: { ledd: string; forklaring: string; verdi: string }[] = [
  {
    ledd: 'BP-brutto for måneden',
    forklaring: 'formen kommer fra planen — sesongen står allerede der',
    verdi: '1 340 000',
  },
  {
    ledd: '× andel av salget som er kommet',
    forklaring: 'nivået kommer fra kassa, ikke fra planen',
    verdi: '0,981',
  },
  {
    ledd: '× kalibrering',
    forklaring: 'treffer stasjonen marginen sin? Læres av seks avlagte måneder',
    verdi: '0,974',
  },
  {
    ledd: '− svinn over det normale',
    forklaring: 'normalt svinn ligger allerede i kalibreringen',
    verdi: '−4 210',
  },
  {
    ledd: '+ bilvask',
    forklaring: 'abonnementene kassa aldri ser',
    verdi: '+3 645',
  },
]
