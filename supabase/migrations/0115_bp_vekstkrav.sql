-- =====================================================================
-- v_bp_status_avdeling: hvor mye vekst KREVER planen?
--
-- Sida viste avviket mot forventningen uten aa vise forventningen. Da
-- kan ikke butikksjefen skille to helt ulike situasjoner:
--
--   * Avdelingen svikter. Da handler det om drift, bemanning, aapningstid.
--   * Planen krevde vekst som aldri kom. Da handler det om planen, og
--     samtalen gaar til St1 sentralt - ikke til henne om innsatsen.
--
-- BILVASK paa Lone, august 2026, er begge deler samtidig:
--
--   BP hele august        71 209
--   Fjoraaret hele august 54 131   -> planen krever +32 %
--   Salg 1.-20.           18 742   -> ned 42 % mot samme dager i fjor
--   Mot forventet                     -56 %
--
-- De to legger seg oppaa hverandre. Uten `bp_vekst_pst` staar bare -56 %
-- igjen paa skjermen, og da leses HVERT ambisiost budsjett som en
-- fiasko i drift.
--
-- MAALT MOT HELE MAANEDEN, IKKE MOT «HITTIL». Vekstkravet er en
-- egenskap ved planen og skal ikke bevege seg med dagteljaren: da ville
-- den samme planen sett ut som et annet krav hver morgen.
--
-- `create or replace` holder her - begge kolonnene legges til SIST, og
-- det er det eneste `or replace` godtar. 0114 maatte droppe viewet
-- fordi `kobling` kom inn midt i.
-- =====================================================================
