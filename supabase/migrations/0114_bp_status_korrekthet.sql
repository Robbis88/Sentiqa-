-- =====================================================================
-- v_bp_status_avdeling: tre korrekthetsrettelser
--
-- 0113 ble kjort mot produksjon og ga tall som saa gale ut: naesten hver
-- avdeling paa naesten hver stasjon laa 20-55 % bak plan. Tre ting i
-- konstruksjonen kunne produsere det uten at driften hadde gjort noe.
--
-- 1. EN DATO FOR HELE KJEDEN. `naa` regnet `max(dato)` over hele
--    v_butikksalg og kryssjoinet den paa hver rad. Ligger en stasjon
--    etter med importen, maales den mot en dag den ikke har rapportert,
--    og da ligger HVER avdeling dens like mye bak.
--
--    Maalt i produksjon 2026-08-21: alle fem stasjoner sto paa
--    2026-08-20, saa dette forklarte IKKE avvikene den dagen. Det er
--    likevel feil - stasjon A skal aldri maales mot stasjon B sin
--    kalender - og det er en feil som ikke ville vist seg foer en
--    import stoppet, altsaa presis naar noen trengte tallet.
--
-- 2. BRUTTOPROSENT UTEN NEVNER. `DRIFT` viste regnskap YTD paa
--    -3536,4 % og et gap paa 3523,9 prosentpoeng. Det er ikke en
--    margin, det er en divisjon paa en omsetning naer null. Slike tall
--    skal ikke regnes; de skal vaere tomme.
--
-- 3. BUDSJETTLINJER UTEN SALG BLE KALT «INGEN PLAN». `211 Selvvask`,
--    `DRIFT` og `SYSTEM` har budsjett, men ingen avdeling aa moete i
--    salgsdataene - `gruppe_navn` viste raa `post`-tekst, som er selve
--    beviset paa at koblingen ikke traff. Sida leste dem som «ingen
--    plan lagt inn», altsaa stikk motsatt: de HAR plan, det er salget
--    som mangler.
--
--    Viewet skiller dem naa i fire, og sier hvilken det er. Hva de skal
--    telle som i totalsummen er en produktbeslutning som ikke tas her -
--    men den kan ikke tas foer kategorien er kjent.
-- =====================================================================

-- `drop` og ikke `create or replace`: viewet faar en ny kolonne, og
-- `or replace` godtar bare kolonner som legges til sist. Ingenting
-- avhenger av viewet, saa dropen er trygg - og `if exists` gjor at hele
-- settet 0001 -> fortsatt taaler aa kjores om igjen.
drop view if exists public.v_bp_status_avdeling;

create view public.v_bp_status_avdeling
...
comment on view public.v_bp_status_avdeling is
  ...
  'Hver stasjon maales mot SIN egen siste salgsdato. Bruttoprosent og '
  'gap er null naar nevneren ikke gir et bruttomaal. `kobling` sier om '
  'budsjett og salg faktisk moettes, eller hvilken av dem som mangler.';
